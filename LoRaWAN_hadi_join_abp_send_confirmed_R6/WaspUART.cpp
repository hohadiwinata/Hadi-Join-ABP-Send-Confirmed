								...
								uint32_t timeout)
{
	...
	uint16_t i = 0;
        ...
        USB.print(F("[debug] UART Baud:")); USB.println(_baudrate);
  	#endif
	...
	//USB.print(F("Read Buffer Answer: ")); USB.println(readBuffer(7));
	_length = 0;
    ...
    while( (millis() - previous) < timeout )
    {
        ...
        //parseString(_data, 10, "/r/n");USB.print(F("[Debug] Current Buffer: ")); USB.println(_data);
		// Check 'ans2'
		if( ans2 != NULL )
		{
							...
							uint32_t timeout )
{
    ...
    while( (millis() - previous) < timeout )
    {
        USB.print(F("[Debug] Serial Availability: ")); USB.println(serialAvailable(_uart));
        ...
        parseString(_data, 10, "/r/n");USB.print(F("[Debug] Wait Buffer: ")); USB.println(_data);
