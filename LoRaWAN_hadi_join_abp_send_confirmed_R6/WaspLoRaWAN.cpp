...
uint8_t WaspLoRaWAN::ON(uint8_t socket)
{
	...
	if (socket == SOCKET0)
	{
        ...
        #if UART_DEBUG > 1
            USB.print(F("[debug] UART is Socket:")); USB.println(SOCKET0);
        #endif
	...
	else
	{
        ...
        #if UART_DEBUG > 1
            USB.print(F("[debug] UART is Socket 1"));
        #endif
...
uint8_t WaspLoRaWAN::getDeviceEUI()
{
	...
	else if (status == 1)
	{
		...
		if (pch != NULL)
		{
            USB.print(F("[debug] LoRaWAN.cpp pch:"));
            USB.println(pch);
			memset(_devEUI,0x00,sizeof(_devEUI));
...
uint8_t WaspLoRaWAN::joinOTAA()
{
	...
	//send command and wait for ans
	status = sendCommand(_command,ans1,ans2,ans3,5000);
...
/*	for (uint8_t i=0;i<strlen(payload);i++)
	...
	if (status == 1)
	{
        ...
        //status = waitFor(ans1, ans2, ans3, ans4, 5000);
        status = waitFor(ans1, 5000);
		//if (waitFor(ans1, 5000) == 1)
		if(status == 1)
		{
			waitFor("\r\n",500);
			if (_length > 0)
...
uint32_t WaspLoRaWAN::parseIntValue()
{
	...
	if (pch != NULL)
	{
		USB.print(F("[debug] LoRaWAN.cpp _buffer:"));
